...
Resolve this skill's directory, then run the bundled script:
...
If the project type cannot be detected, provide `--commands`. The generated
`init.sh` intentionally fails until real verification commands are configured.
